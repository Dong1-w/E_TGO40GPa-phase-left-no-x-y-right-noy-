...
TimerOutput::Scope ts(computing_timer, "setup_mesh");
    ...
    GridGenerator::subdivided_hyper_rectangle(triangulation, repetitions, p1, p2, true);
        ...
        // Geometric tolerance scales with domain size for robust boundary tagging.
    constexpr double domain_span =
      ((Domain::x_max - Domain::x_min) > (Domain::y_max - Domain::y_min)) ?
      (Domain::x_max - Domain::x_min) : (Domain::y_max - Domain::y_min);
    constexpr double boundary_id_tolerance = 1e-12 * domain_span;
...
double Y0 = 0.5 * local_sigma_c * local_sigma_c / E;  //sigma_c��λ��Pa
...
// �����Ŷ������ڹ���Ԥ�ڵ�����λ�� (x=4.0mm ����)
// �����Ŷ�ʱ�䣺�������˳���ʩ�ӣ�һ�����ο�ʼ���Ƴ�������һֱ����΢Сֵ
bool is_center_region = (std::abs(cell_center[0] - 4.0) < 0.5); // ���� 1mm ����������
bool is_top_surface = (std::abs(cell_center[1] - Domain::y_tc_top) < 0.05); // �����ϱ���
...
// ���������ϱ�����ʱ�����ʱʩ�ӣ����ߵ�λ�ƻ���Сʱʩ��
if (is_center_region && is_top_surface && time < 100000.0) // ������ǰ1000����ʩ��
{
perturbation_pressure = 0.1; // 1 MPa ������ǣ����
}
// ���ߣ�ʼ��ʩ��һ����С����??������
// perturbation_pressure = 0.1; 

// ���Ŷ����ӵ��в������� (RHS)
// ע�⣺elasticity �� RHS �� -Residual���������� F Ӧ���� +F ����ʽ�ӽ�ȥ
// ��Ӧ������ʽ���ǣ� - integral( sigma : grad_v ) + integral( f * v ) = 0
// ���� Residual = integral( sigma : grad_v ) - integral( f * v )
// ���� RHS = -Residual = -integral( sigma : grad_v ) + integral( f * v )
...
double local_max_abs_change_phi = 0.0; // �洢�ೡ�����������Ա仯��

// 1. ����Ũ�ȵı仯������ԭ���߼������ھ��Ի���Ա仯���������Ũ����Ȼ��ע�仯���ȣ�
...
data_out.add_data_vector(dof_handler_phase_field, locally_relevant_solution_phase_field, "phi");
...
{
// ��¼��ǰ����ʱ��Ͷ�Ӧ���ļ���
// ע�⣺write_vtu_with_pvtu_record ���ɵ��ļ�����ʽͨ���� "solution_����.pvtu"
...
{
double local_max_tgo_sigma1 = -1e20; // ��ʼ��Ϊһ����С����

// �������б���ӵ�еĵ�Ԫ
unsigned int local_cell_index = 0; // ���Ա���ӵ�е�Ԫ��������Ӧ principal_stress_*
for (const auto &cell : triangulation.active_cell_iterators())
{
if (!cell->is_locally_owned())
continue;
...
if (layer == TGO)
{
// ��֮ǰ����õ� principal_stress_1 �����л�ȡ�õ�Ԫ��Ӧ��
// principal_stress_1 �洢����ÿ����Ԫ�ڲ����ֵ�������Ӧ�������ֵ
...
{
pcout << "CMAS Phase Field Corrosion Simulation (Parallel Version) - REVISED" << std::endl;
pcout << "  Using Newton-Raphson method for phase field equation" << std::endl;
pcout << "=========================================================" << std::endl;
...
// === �޸Ŀ�ʼ���� time loop �ⲿ����������� ===
const unsigned int max_outer_iterations = 1; // ����ⲿ��������
const double outer_tolerance = 5e-2;          // �ⲿ�����ݲ� (phi �ı仯��)
// === �޸Ľ��� ===
...
// �ڽ������ǰ������ phi_prev_iter ������һʱ�䲽�Ľ�(��ʼ�²�)
phi_prev_iter = old_solution_phase_field;

while (outer_iter < max_outer_iterations && error_phi > outer_tolerance)
{
// A. ��ⵯ�Գ� (���ڵ�ǰ���µ� phi)
// ע�⣺assemble_elasticity_system �ڲ����ȡ locally_relevant_solution_phase_field
assemble_elasticity_system();
solve_elasticity();

// B. ������ʷ�� H
// �ೡ�ݻ������� max(H_history, current_elastic_energy)
// ÿ�ε��Գ����º�Ӧ���ܱ��ˣ����Ա��������ʷ�����ೡ�����ʹ��
update_history_field(); 

// ��¼���ε���ǰ�� phi�����ڼ������
// ע�⣺����Ҫ�Ƚϵ���"����Newton�����phi"��"��һ���ⲿ������phi"
phi_prev_iter = completely_distributed_solution_phase_field;

// C. ����ೡ (Newton-Raphson �ڲ�����)
// ������ø��º�� history_field �����µ� phi
solve_phase_field_newton();

// D. �����������: || phi_new - phi_prev ||_inf
...
diff_vec.sadd(1.0, -1.0, phi_prev_iter); // diff = current - prev

double local_error = diff_vec.linfty_norm();
error_phi = Utilities::MPI::max(local_error, mpi_communicator);

pcout << "    [Outer Iter " << outer_iter + 1 << "] Phi correction error: " << error_phi << std::endl;

outer_iter++;
}

if (outer_iter >= max_outer_iterations) {
pcout << "    WARNING: Staggered iteration did not converge fully." << std::endl;
} else {
...
pcout << "  [MONITOR] Max Principal Stress in TGO: " << max_tgo_sigma << " MPa" << std::endl;
...
// update_history_field() �Ѿ���ѭ����ִ�й��ˣ����ﲻ��Ҫ�ٴ�ִ��
// ֻ��Ҫ����ǰ���ĽⱣ��Ϊ old_solution ����һ��ʹ��
old_solution_concentration = locally_relevant_solution_concentration;
old_solution_phase_field = locally_relevant_solution_phase_field;
