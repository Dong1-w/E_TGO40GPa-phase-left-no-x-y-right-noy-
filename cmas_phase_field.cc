...
// === 修改开始：在 time loop 外部定义迭代参数 ===
const unsigned int max_outer_iterations = 1; // 最大外部迭代次数
const double outer_tolerance = 5e-2;          // 外部迭代容差 (phi 的变化量)
// === 修改结束 ===
...
// 备份当前步开始时的相场解，用于计算每一步迭代的修正量
LA::MPI::Vector phi_prev_iter;
phi_prev_iter.reinit(locally_owned_dofs_phase_field, mpi_communicator);

// 在进入迭代前，先让 phi_prev_iter 等于上一时间步的解(初始猜测)
phi_prev_iter = old_solution_phase_field;

while (outer_iter < max_outer_iterations && error_phi > outer_tolerance)
{
// A. 求解弹性场 (基于当前最新的 phi)
// 注意：assemble_elasticity_system 内部会读取 locally_relevant_solution_phase_field
assemble_elasticity_system();
solve_elasticity();

// B. 更新历史场 H
// 相场演化依赖于 max(H_history, current_elastic_energy)
// 每次弹性场更新后，应变能变了，所以必须更新历史场供相场求解器使用
update_history_field(); 

// 记录本次迭代前的 phi，用于计算误差
// 注意：我们要比较的是"本次Newton算出的phi"和"上一次外部迭代的phi"
phi_prev_iter = completely_distributed_solution_phase_field;

// C. 求解相场 (Newton-Raphson 内部迭代)
// 这会利用更新后的 history_field 计算新的 phi
solve_phase_field_newton();

// D. 计算收敛误差: || phi_new - phi_prev ||_inf
